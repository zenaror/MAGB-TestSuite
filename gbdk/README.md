...
at the actual repo root (one level up from this `gbdk/` directory --
shared with any other implementation directory, not copied per
implementation). **This file is not part of the repository** (it's
...
`../config.bin` (relative to here). Without it, those specific checks
are skipped with a clear message rather than failing; the rest of
`make test` still runs normally.
...
5. **P2P Caller** / **P2P Listener** — two roles of the same test,
   ...
   (libmobile's direct-IP P2P path: the dialed 12-digit number is
   parsed as an IPv4 address and connected to straight over TCP — see
   `docs/testing.md` for the port/reachability requirements, and why a
   real relay-based P2P call is a different, adapter-level mechanism
   this test doesn't exercise). Exchanges a small deterministic binary
